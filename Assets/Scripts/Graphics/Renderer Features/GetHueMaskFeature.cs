...
public class GetHueMaskFeature : ScriptableRendererFeature {

    [System.Serializable]
    public class CustomSettings { // ScriptableRenderPassSettings

        public string passName = "Get Hue Mask Pass";

        public RenderPassEvent renderPassEvent = RenderPassEvent.BeforeRenderingPostProcessing;
        [HideInInspector] public Material material;

        [Range(0f, 1f)] public float hueLeft;
        [Range(0f, 1f)] public float hueRight;
        [Range(0f, 1f)] public float saturationThesh;
        [Range(0f, 1f)] public float valueThresh;
        public bool blackNWhite = false;

        public void OnSetup(ref RenderingData renderingData) {
            material.SetFloat("_HueLeft", hueLeft);
            material.SetFloat("_HueRight", hueRight);
            material.SetFloat("_SaturationThreshold", saturationThesh);
            material.SetFloat("_ValueThreshold", valueThresh);
            material.SetFloat("_RemoveColor", blackNWhite ? 1f : 0f);
