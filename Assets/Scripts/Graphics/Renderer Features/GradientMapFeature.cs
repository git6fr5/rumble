...
public class GradientMapFeature : ScriptableRendererFeature {

    [System.Serializable]
    public class CustomSettings { // ScriptableRenderPassSettings

        public string passName = "Gradient Map Pass";
        ...
        public bool twoTone;
        public bool keepBlack;

        public void OnSetup(ref RenderingData renderingData) {
            material.SetTexture("_GradientMap", gradient);
            material.SetFloat("_ColorCount", gradient.width);
            material.SetFloat("_TwoTone", twoTone ? 1f : -1f);
            material.SetFloat("_KeepBlack", keepBlack ? 1f : -1f);
