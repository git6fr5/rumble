...
public class Platform : MonoBehaviour {
    ...
    protected virtual void Init() {
        ...
        box.size = new Vector2(endPoint.localPosition.x, 0.5f);
        box.offset = new Vector2((endPoint.localPosition.x - 1f) / 2f, 0.25f);
    ...
    public static float Factor = 20f;
    protected virtual void Target() {
        // 

        Rigidbody2D body = GetComponent<Rigidbody2D>();
        if (transform.position.y < target.y - GameRules.MovementPrecision) {
            body.velocity = Vector3.up * (target.y - transform.position.y) * Factor;
        }
        else {
